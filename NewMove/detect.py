...
from NewMove.functions.get_param_groups import get_param_groups
from NewMove.module.detect.model.common import Conv
from NewMove.module.detect.model.yolo import Model
from NewMove.module.detect.utils.general import labels_to_class_weights, non_max_suppression
from NewMove.module.detect.utils.loss import ComputeLoss
from NewMove.module.detect.utils.metrics import box_iou
...
class Detect:
    ...
    def __init__(self, config, mode: Literal['train', 'inference'], nc: int, classes: List[str], labels: List[Tensor]):
        ...
        config_p = Path(__file__).parents[1] / 'NewMove' / 'module' / 'detect' / 'model' / f'{model_t}.yaml'
    ...
    def load_ckpt(self, ckpt: dict):
        ...
        self.net.load_state_dict(ckpt,False)
    ...
    @torch.no_grad()
    def eval(self,  sample:List, imgs: Tensor, targets: Tensor, stats: List, preview: bool = True) -> Tuple[int, Tensor | None]:
        self.net.eval()
        ...
        # preview
        if preview:
            prv = self.preview(sample, imgs, preds)
            return seen, prv
    ...
    @staticmethod
    def preview(sample: List, imgs: Tensor, preds: Tensor, conf_th: float = 0.6):
        imgs_mk = []
        ...
        for index in range(len(imgs_mk)):
            image = imgs_mk[index].cuda().clone()
            image = image.squeeze(0)
            image = unloader(image)
            image.save(f'detect_pictures/detect_{sample[index].split(".")[0]}.jpg')
        ...
        imgs_c = []
        for i in range(3):
            t = [imgs_mk[i * 3 + j] for j in range(3)]
            imgs_c.append(torch.cat(t, dim=2))
        imgs_one = torch.cat(imgs_c, dim=1)
