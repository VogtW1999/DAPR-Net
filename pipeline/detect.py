...
class Detect:
    ...
    @torch.no_grad()
    def eval(self,  sample:List, imgs: Tensor, targets: Tensor, stats: List, preview: bool = False) -> Tuple[int, Tensor | None]:
        self.net.eval()
    ...
    def preview(sample: List, imgs: Tensor, preds: Tensor, conf_th: float = 0.6):
        ...
        for index in range(len(imgs_mk)):
            image = imgs_mk[index].cuda().clone()  # clone the tensor
